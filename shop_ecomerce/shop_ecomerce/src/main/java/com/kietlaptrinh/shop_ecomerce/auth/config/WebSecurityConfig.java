...
import com.kietlaptrinh.shop_ecomerce.auth.exceptions.RESTAuthenticationEntryPoint;
...
public class WebSecurityConfig {
    ...
    private static final String[] publicApis= {
        "/api/auth/**",
            "/api/order/paypal/success",
            "/api/order/paypal/cancel"
};
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http)throws Exception{
        http.csrf(AbstractHttpConfigurer::disable)
                ...
                .authorizeHttpRequests((authorize)->authorize
                        // Allow public access to specific API documentation
                .requestMatchers("/v3/api-docs/**", "/swagger-ui.html", "/swagger-ui/**").permitAll()
                .requestMatchers(HttpMethod.GET,"/api/products","/api/category").permitAll()
                        .requestMatchers("/api/order/paypal/success", "/api/order/paypal/cancel").permitAll()
